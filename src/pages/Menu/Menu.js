...
export default function Menu() {
  ...
  const columns = [
...

    {
      ...
      dataIndex: "menu_id",
      align: "center",
      render: (value, record) => <Tag color="#2db7f5">{record.menu_id}</Tag>,
    },
    {
      title: "上级菜单",
      dataIndex: "pid",
      align: "center",
      render: (value, record) =>
        record.pid ? <Tag color="#2db7f5">{record.pid}</Tag> : null,
    },
    { title: "路径", dataIndex: "path", align: "center" },
    {
      ...
      dataIndex: "isLeaf",
      align: "center",
      render: (value, record) => <span>{record.isLeaf ? "不是" : "是"}</span>,
    },
      ...
      render: (value, record) => {
        ...
        return (
          <div className="flex justify-between">
            <AuthButton auth="menu:r">
              <Button type="primary" onClick={() => handleView(_id)}>
                查看
              </Button>
            </AuthButton>
            <AuthButton auth="menu:u">
              <Button
                type="primary"
  ...
  const Query = () => (
    ...
    >
      <Row>
        <Col span={6}>
          <Form.Item label="是否为目录" name="isLeaf" labelCol={{ span: 8 }}>
            <Radio.Group>
              <Radio.Button value={0}>是</Radio.Button>
              <Radio.Button value={1}>不是</Radio.Button>
            </Radio.Group>
          </Form.Item>
        </Col>
        <Col span={6}>
          ...
          <Form.Item name="pid" label="上级菜单">
            <Select placeholder="选择上级菜单" allowClear>
              {option.map((item) => {
                return (
